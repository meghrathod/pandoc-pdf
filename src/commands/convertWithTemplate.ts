...
export async function convertWithTemplate(): Promise<void> {
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const result = await converter.convertWithCustomTemplate(filePath, selectedTemplate.label);
            ...
            if (result.success && result.outputPath) {
                ...
                logError(result.error || 'Unknown error', 'convertWithTemplate');
            }
        } catch (error: any) {
            const parsedError = parsePandocError(error.message);
            showErrorDialog(parsedError);
            logError(error.message, 'convertWithTemplate');
        }
    });
}
